...
stat_t nexti(y64sim_t *sim)
{
	...
	regid_t iregA = GET_REGA(codefun1);
	regid_t iregB = GET_REGB(codefun1);
	next_pc++;
	long_t codefun2 = 0;
	if(!get_long_val(sim->m,next_pc,&codefun2)){//instructions are placed at memory
	  err_print("PC = 0x%x, Invalid instruction address", sim->pc);
	  return STAT_ADR;
	}
	long_t iime=codefun2;
	next_pc+=8;
	long_t getA = get_reg_val(sim->r, iregA);
	long_t getB = get_reg_val(sim->r, iregB);
	long_t codefun3=0;
	if(!get_long_val(sim->m, getB+iime,&codefun3)){
	  err_print("PC = 0x%x, Invalid data address 0xffffffff%x", sim->pc, getB+iime);
		return STAT_ADR;
	}
